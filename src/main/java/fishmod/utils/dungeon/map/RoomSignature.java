    ...
    public static RoomSignature of(RoomTile tile) {
        ...
        for (RoomTile seg : segments) {
            for (int[] dir : new int[][]{{2, 0}, {-2, 0}, {0, 2}, {0, -2}}) {
                GridPos connectorPos = seg.pos().offset(dir[0] / 2, dir[1] / 2);
                Tile connector = DungeonGrid.get(connectorPos);
                if (connector instanceof DoorTile door && door.state() != RoomState.UNDISCOVERED) {
                    doorOffsets.add(new int[]{seg.pos().x() + dir[0] / 2, seg.pos().z() + dir[1] / 2});
                }
            }
        }
