...
public final class ParticleCosmetics {
    ...
    public enum PType {
        ...
        HAPPY("Happy", ParticleTypes.HAPPY_VILLAGER), DRAGON("Dragon", ParticleTypes.DRAGON_BREATH),
    ...
    private static void spawn(MinecraftClient mc, ClientPlayerEntity p) {
        ...
        switch (FishSettings.particleStyle) {
            case TRAIL -> {
                ...
                mc.world.addParticle(fx, x + rand(0.25), y + 0.05, z + rand(0.25), 0, 0.01, 0);
            }
            case AURA -> {
                // Two points orbiting at waist height.
                for (int i = 0; i < 2; i++) {
                    double a = tickAngle + i * Math.PI;
                    mc.world.addParticle(fx, x + Math.cos(a) * 0.7, y + 1.0, z + Math.sin(a) * 0.7, 0, 0, 0);
                }
            }
            case WINGS -> {
                ...
                for (int i = 1; i <= 3; i++) {
                    ...
                    mc.world.addParticle(fx, x + bx * back + sx * span, y + up, z + bz * back + sz * span, 0, 0, 0);
                    mc.world.addParticle(fx, x + bx * back - sx * span, y + up, z + bz * back - sz * span, 0, 0, 0);
                }
            }
            case FOOTSTEPS -> {
                ...
                if (dx * dx + dz * dz > 0.0006 && p.isOnGround())
                    mc.world.addParticle(fx, x + rand(0.15), y + 0.02, z + rand(0.15), 0, 0, 0);
            }
        }
        lastX = x; lastZ = z;
    }
