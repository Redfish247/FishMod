...
public class DungeonMapHud {
    ...
    /** Centered horizontally across the room's full bounding box, anchored to the top of the room
     *  (not vertically centered — Eli wants names readable at the top rather than overlapping the
     *  room's center where doors/markers sit), and scaled down to fit if the name is too wide. */
    private static void drawLabel(DrawContext ctx, int boxX, int boxY, int boxWidth, String text, int color) {
        ...
        Matrix3x2fStack stack = ctx.getMatrices();
        stack.pushMatrix();
        stack.translate(boxX + boxWidth / 2f, boxY + 1);
