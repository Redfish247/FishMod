...
public class PartyCommandHandler {
    ...
    public static void onPartyCommand(String typer, String cmd, String rawArg1, String rawArg2, String responder) {
        ...
        switch (cmd) {
            case "help", "?" -> { if (FishSettings.pcHelp && shouldRespond(cmd, typer)) sendCmd(mc, responder + buildHelp()); }
            // Stats lookups: respond to ANY party member's command (default target = typer if no arg)
            case "rtca"      -> { if (FishSettings.pcRtca && shouldRespond(cmd, typer))    runRtcaForPlayer(mc, ign, responder);             }
            case "rtc"       -> { if (FishSettings.pcRtc  && shouldRespond(cmd, typer)) {
            ...
            case "cata"      -> { if (FishSettings.pcCata && shouldRespond(cmd, typer))    runCataForPlayer(mc, ign, responder);             }
            case "pb" -> {
                if (!FishSettings.pcPb || !shouldRespond(cmd, typer)) break;
            ...
            case "mp" -> { if (FishSettings.pcMp && shouldRespond(cmd, typer)) runMpForPlayer(mc, rawArg1 != null ? rawArg1 : typer, responder); }
            case "collection" -> {
                if (!FishSettings.pcCollection || !shouldRespond(cmd, typer)) break;
            ...
            case "secrets", "sa" -> { if (FishSettings.pcSecrets && shouldRespond(cmd, typer)) runStatsForPlayer(mc, ign, cmd, null, responder); }
            case "runs" -> {
                if (!FishSettings.pcRuns || !shouldRespond(cmd, typer)) break;
            ...
            case "totalruns" -> { if (FishSettings.pcRuns && shouldRespond(cmd, typer))    runTotalRunsForPlayer(mc, ign, responder);        }
            ...
            case "corpse", "corpses" -> { if (FishSettings.pcCorpse && isMe) sendCorpse(mc, responder);  }
            case "bank" -> { if (FishSettings.pcBank && isMe) sendBank(mc, responder); }
            case "nw", "networth" -> { if (FishSettings.pcNw && shouldRespond(cmd, typer)) sendNetworth(mc, ign, responder); }
    ...
    public static boolean handleCommand(String fullCmd) {
        ...
        switch (cmd) {
            ...
            case "d":
                ...
                return true;
            case "bank":
                if (!FishSettings.pcBank) return false;
                sendBank(mc, responder);
                return true;
            case "corpse": case "corpses":
                if (!FishSettings.pcCorpse) return false;
                sendCorpse(mc, responder);
                return true;
    ...
    private static void sendDprofit(MinecraftClient mc, String responder) {
        java.util.List<blade.addon.features.croesus.CroesusStore.Entry> all = blade.addon.features.croesus.CroesusStore.all();
        String last = all.isEmpty()
                ? "—"
                : blade.addon.features.croesus.CroesusOverlay.bestDrop(all.get(all.size() - 1));
        String pr = blade.addon.features.croesus.CroesusOverlay.fmtCoins(
                blade.addon.features.croesus.CroesusOverlay.avgProfit());
    ...
    private static void sendCorpse(MinecraftClient mc, String responder) {
        HypixelApi.getEconomy(mc, (bank, purse, corpses) ->
            sendCmd(mc, responder + "Corpses: " + (corpses != null ? corpses : "N/A")));
    }

    private static void sendBank(MinecraftClient mc, String responder) {
        HypixelApi.getEconomy(mc, (bank, purse, corpses) -> {
            String b = bank >= 0 ? fmtCoins(bank) : "N/A";
            String p = purse >= 0 ? fmtCoins(purse) : "N/A";
            sendCmd(mc, responder + "Bank: " + b + " | Purse: " + p);
        });
    }

    private static void sendNetworth(MinecraftClient mc, String ign, String responder) {
        blade.addon.features.croesus.CroesusPrices.refreshIfStale();
